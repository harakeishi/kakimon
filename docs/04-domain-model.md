...
interface Monster {
  ...
  stats: {
    maxHp: number;
    hp: number;            // 現在HP。下限 1。1 まで落ちると lifeState が "dying" へ
    attack: number;
    defense: number;
    speed: number;
  ...
  /** 健康の段階。モンスターは死なないので最下段は "dying"。詳細は 4.4 */
  lifeState: "healthy" | "weak" | "sick" | "dying";
...
おり、`settings` テーブルに singleton（id: `"room"`）で保存する。新しいタマゴでの
...
状態で、`settings` テーブルに singleton（id: `"loginBonus"`）で保存する。新しいタマゴでの
...
オフライン中の時間経過をどう扱うか。長期間世話をしないとモンスターは
段階的に弱っていく。ただし**モンスターは死なない**。悪化は「危篤」で
止まり、そこから先は無い。いつ戻ってきても、世話をすれば必ず回復できる。
...
**モンスターは死なない。** 悪化は `dying`（危篤）で止まり、そこから先は無い。
世話をすれば必ず戻ってこられる。

```
                  良い世話                  悪化           回復         悪化
  healthy ──────────────────▶ weak ────▶ sick ────▶  ...  ────▶ dying ──┐
     ▲                          │           │                     │      │
     │       回復(餌+撫で)       │           │  回復(餌+お風呂)    │      │ ここで打ち止め
     └──────────────────────────┴───────────┴─────────────────────┘◀─────┘
```
...
| `dying`   | hp が下限（1）まで落ちた瞬間。`dyingSince` を記録 | 大きな警告。学習を一旦止めて世話を促す。ここより悪くはならない |
...
- `hp`: `sick` / `dying` 中のみ 24 時間で maxHp の 25% 減。`weak` では減らない。
  ただし **hp の下限は 1**。0 にならないので HP 切れで死ぬことはない。
- `hp`（回復）: `sick` / `dying` でも hunger < 60（ちゃんと食べている）なら
  同じ速さ（24 時間で maxHp の 25%）で回復に転じる。
- これらは「世話されない放置時間」を基準にしており、撫でる・給餌・お風呂で
  即座に回復する。
- 危篤の子にごはんをあげると hp を maxHp の 25% まで戻して `sick` に復帰する。
...
```
  0h   ─ healthy
 24h   ─ hunger MAX、weak → sick へ移行
 24h ~ 168h ─ hp が徐々に減少 (約 4 日で maxHp → 下限 1)
168h   ─ dying へ移行（≒ 1 週間放置）
それ以降 ─ dying のまま。どれだけ放置しても死なない
```

数字は調整前提。デザインの意図は「夏休みの旅行（1 週間）で帰ってきたら危篤」
程度の余裕。学校に行っている間は確実に間に合う。
...
死亡は無いので自動で再スタートが発生することはない。ユーザ（保護者）が
明示的に「新しいタマゴで はじめる」を選んだときだけ以下を行う：

1. 大きく演出はせず、優しい「ありがとう」画面を見せる。
2. 当該 Monster は **図鑑（コレクション）におもいでとして残す**。名前・誕生日・
...
- **モンスターは死なない**。世話を怠ると弱っていくが、`dying`（危篤）で
  止まり、いつでも世話で回復できる。小さい子が使う学習アプリで、
  「放置していたら消えていた」という体験は作らない。
- 弱る過程は残す。世話の意味（ごはん・撫でる）と「ちゃんと見てあげよう」という
  動機づけは、喪失ではなく回復のうれしさで作る。
- 学習履歴・コインは何があっても失わせない。学習の努力とペットの状態は別軸。
- 「家出」「破壊」など他の喪失バリエーションも入れない。喪失は 1 つも持たない。
- 個体の入れ替えは、ユーザが明示的に選んだときだけ（「新しいタマゴで はじめる」）。
