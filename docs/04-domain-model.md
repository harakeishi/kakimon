...
`species` は現在 `chick`（ことり）/ `dragon`（ドラゴン）/ `cat`（ねこ）の
3 種類。孵化前のタマゴで選択し、孵化後は変更しない。各種族は同じ成長段階を
持つが、段階ごとの見た目は `domain/monsterSpecies.ts` のカタログで個別に定義する。
...
学習で経験値を獲得してレベルアップすると、次のレベルで自動的に進化する。
...
1 回の報酬で複数の閾値を越えた場合も、到達レベルに対応する段階まで進化する。
種族を問わず閾値は共通で、見た目だけが種族ごとに変わる。
